  ...
  <div class="fund-card" @click="fundItemClick(fund.prdNo)">
    <div class="fund-content">
      ...
      <div class="fund-info">
        ...
        <p class="fund-nav">
          <b>기준가:</b> <span class="highlight">{{ fund.nav }}</span>
        </p>
      </div>
...
export default {
  ...
  methods: {
    ...
    saveCheckboxState() {
      const checkedFunds =
        JSON.parse(localStorage.getItem("checkedFunds")) || [];
      if (this.isChecked) {
        const fundExists = checkedFunds.some(
          (fund) => fund.prdNo === this.fund.prdNo
        );
        if (!fundExists) {
          // 펀드 전체 데이터를 저장
    ...
    loadCheckboxState() {
      const checkedFunds =
        JSON.parse(localStorage.getItem("checkedFunds")) || [];
      this.isChecked = checkedFunds.some(
        (fund) => fund.prdNo === this.fund.prdNo
      );
    },
  },
