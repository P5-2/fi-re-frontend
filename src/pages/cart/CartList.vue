    ...
    <div class="header">
      ...
      <div class="tab-buttons">
        ...
        <button
          ...
          :class="{ active: selectedTab === 'saving' }"
        >
          적금
        </button>
        <button
          @click="selectedTab = 'fund'"
          :class="{ active: selectedTab === 'fund' }"
        >
          펀드
        </button>
      </div>
      ...
      <button
        ...
        :disabled="
          (selectedTab === 'fund' &&
            (selectedFundsItems.size === 0 || selectedFundsItems.size > 3)) ||
          (selectedTab === 'saving' &&
            (selectedSavingsItems.size === 0 ||
              selectedSavingsItems.size > 3)) ||
          (selectedTab === 'deposit' &&
            (selectedDepositsItems.size === 0 ||
              selectedDepositsItems.size > 3))
        "
      >
        비교하기 ({{
          selectedTab === "fund"
            ? selectedFundsItems.size
            : selectedTab === "saving"
            ? selectedSavingsItems.size
            : selectedDepositsItems.size
        }}/3)
      </button>

      <button @click="removeSelectedItems" class="remove-btn">
        선택한 항목 삭제
      </button>
    </div>
    ...
    <div v-if="currentItems.length">
      ...
      <ul v-if="selectedTab === 'deposit'">
        ...
        <DepositItem
          ...
          :item="item"
          :isSelected="
            selectedDepositsItems.has(
              `${item.savingsDeposit.fin_prdt_cd}-${item.options[0].intr_rate_type_nm}`
            )
          "
      ...
      <ul v-if="selectedTab === 'fund'">
        <FundItem
          ...
          :key="index"
          :item="item"
          :isSelected="selectedFundsItems.has(item.prdNo)"
          @update-selected-items="updateSelectedItemsFund"
        />
      </ul>
    </div>
    <div v-else>
...
import {
  loadFundsInCart,
  removeFundFromCart,
} from "@/services/cartServiceFund.js";
import {
  loadSavingsDepositByUsername,
  loadDepositByUsername,
  removeDepositFromCart,
  removeSavingsFromCart,
} from "@/services/cartServiceSD"; // 서비스 파일 임포트
    ...
    updateSelectedData(type) {
      if (type === "fund") {
        this.selectedFunds = this.funds.filter((fund) =>
          this.selectedFundsItems.has(fund.prdNo)
        );
      } else if (type === "saving") {
        ...
        for (const item of this.savings) {
          ...
          if (this.selectedSavingsItems.has(itemKey)) {
            await removeSavingsFromCart(
              username,
              item.savingsDeposit.fin_prdt_cd,
