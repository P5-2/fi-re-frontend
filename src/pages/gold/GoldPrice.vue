<template>
  <div class="container mt-4">
    <div class="card bg-light mb-4">
      <div class="card-body">
        <h1>금 시세</h1>
        <label for="endBasDt">기준 날짜: </label>
        <input type="text" v-model="endBasDt" placeholder="ex) 240101" />
        <label for="numOfRows">데이터 양: </label>
        <input type="number" v-model="numOfRows" placeholder="Rows" />
        <button @click="fetchGoldData" class="btn btn-primary">
          데이터 출력
        </button>
        <div>
          <button type="button" class="btn btn-primary" @click="setPeriod(180)">
            6개월
          </button>
          <button type="button" class="btn btn-primary" @click="setPeriod(365)">
            1년
          </button>
          <button type="button" class="btn btn-primary" @click="setPeriod(1095)">
            3년
          </button>
        </div>
      </div>
    </div>
    <!-- 금 시세 데이터가 있을 때만 데이터 출력 및 차트 렌더링 -->
    <div v-if="goldData.length" class="border border-secondary border-opacity-50 p-3 rounded">
      <h2>Gold Prices</h2>
      <!-- 금 시세 차트 -->
...
export default {
  ...
  methods: {
    ...
    async fetchGoldData() {
      try {
        ...
        // console.log('Gold data stored as JSON:', this.jsonData);
      } catch (error) {
    ...
    async fetchPredictedGoldPrice() {
      try {
        ...
        // 로컬 스토리지에 저장
        localStorage.setItem('predictedGoldData', JSON.stringify(predictedData, null, 2));
          ...
          datasets: [
            {
              ...
              pointBackgroundColor: (ctx) => {
                ...
                return index === maxIndex ? 'red' : index === minIndex ? 'blue' : 'rgba(0, 0, 0, 0)';
              },
              pointRadius: (ctx) => {
                const index = ctx.dataIndex;
                return (index === minIndex || index === maxIndex) ? pointRadius : 0;
              },
              pointHoverRadius: (ctx) => {
                const index = ctx.dataIndex;
                return (index === minIndex || index === maxIndex) ? pointRadius * 1.5 : 0;
              },
              segment: {
                borderColor: (ctx) => {
                  const isPredict = combinedData[ctx.p0DataIndex].isPredicted;
                  return isPredict ? 'rgba(66, 245, 81, 1)' : 'rgba(245, 194, 66, 1)';
                },
                backgroundColor: (ctx) => {
                  const isPredict = combinedData[ctx.p0DataIndex].isPredicted;
                  return isPredict ? 'rgba(66, 245, 81, 0.1)' : 'rgba(245, 194, 66, 0.1)';
                }
              },
            },
          ],
  ...
  mounted() {
    ...
    const loadGoldData = async () => {
      if (storedData) {
        const lastElement = JSON.parse(storedData)[JSON.parse(storedData).length - 1];
        if (lastElement) {
